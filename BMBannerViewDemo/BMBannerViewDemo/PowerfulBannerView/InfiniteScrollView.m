...
- (void)layoutSubviews
{
    ...
    // <InfiniteScrollView: 0x7fab11813000; baseClass = UIScrollView; frame = (0 0; 394 200); clipsToBounds = YES; gestureRecognizers = <NSArray: 0x60000005e900>; layer = <CALayer: 0x600000037f20>; contentOffset: {2364, 0}; contentSize: {5122, 200}; adjustedContentInset: {0, 0, 0, 0}>
    ...
    //NSLog(@"......... %lf -- %lf --- %lf", CGRectGetMinX(visibleBounds),  minimumVisibleX, maximumVisibleX);
...
- (CGFloat)placeNewViewOnCenter:(CGFloat)x
{
    ...
    NSLog(@"========>>> centerView: %p", self.centerView);
    ...
    contentView.frame = view.bounds; // 在这里改变子视图的frame的
    [view addSubview:contentView];
    
    return CGRectGetMaxX(frame);
}

// 在containerView的最右侧添加一个视图, 并把这个视图加入到visibleViews数组中
- (CGFloat)placeNewViewOnRight:(CGFloat)rightEdge reusableView:(UIView *)view
{
    ...
    [theView addSubview:cv]; // 把用户的视图添加个这个视图中(小的contentView)
    
    return CGRectGetMaxX(frame);
}

// 在containerView的最左侧添加一个视图, 并把这个视图加入到visibleViews数组中
...
- (void)tileViewsFromMinX:(CGFloat)minimumVisibleX toMaxX:(CGFloat)maximumVisibleX _:(CGRect)visibleBounds
{
    ...
    if (rightEdge < maximumVisibleX) { // visibleViews数组中最后一个视图的最右侧 < maximumVisibleX, 则右侧添加之
        ...
        if (leftEdge > minimumVisibleX) { // visibleViews数组中最后一个视图的最左侧 < minimumVisibleX, 则左侧添加之
    ...
    NSLog(@">>>> %lf", scale);
    int s = round(scale); // 四舍五入,比如-0.52368866328257191就会变成:-1
    
    // NSLog(@"%@", NSStringFromCGRect(self.centerView.frame));
    // NSLog(@"%d", s);
    
    if (s - self.centerViewLastPosition == 1) { // 视图右移超过一半宽度
        
        self.currentIndex = fixIndex(self.currentIndex-1, [self.dataSource viewCycleCount]);
        //NSLog(@"%d ------ %d", s, self.centerViewLastPosition); // -1 ------ 0
        self.centerViewLastPosition = s;
        
    } else if (s - self.centerViewLastPosition == -1) { // 视图左移超过一半宽度
        // NSLog(@"%d -------- %d", (int)self.currentIndex, (int)[self.dataSource viewCycleCount]);
        //NSLog(@"%d ------ %d", s, self.centerViewLastPosition);
        self.currentIndex = fixIndex(self.currentIndex+1, [self.dataSource viewCycleCount]);
        self.centerViewLastPosition = s;
